...
static void setValueFromGdbString(VarWatch *w, QString data)
{
    CoreVar &var = w->getVar();
    QList<Token*> tokenList = GdbMiParser::tokenizeVarString(data);
    QList<Token*> orgList = tokenList;

    GdbMiParser::parseVariableData(&var, &tokenList);
...
int GdbCore::openPseudoTerminal()
{
 ...
 */
void GdbCore::detectMemoryDepth()
{
    Tree resultData;
    GdbCom& com = GdbCom::getInstance();
 ...
 */
int GdbCore::initPid(Settings *cfg, QString gdbPath, QString programPath, int pid)
{
    GdbCom& com = GdbCom::getInstance();
    Tree resultData;
    int rc = 0;
 ...
 */
int GdbCore::runInitCommands(Settings *cfg)
{
    GdbCom& com = GdbCom::getInstance();
...
int GdbCore::initLocal(Settings *cfg, QString gdbPath, QString programPath, QStringList argumentList)
{
    GdbCom& com = GdbCom::getInstance();
    Tree resultData;
    int rc = 0;
 ...
 */
int GdbCore::initCoreDump(Settings *cfg, QString gdbPath, QString programPath, QString coreDumpFile)
{
    GdbCom& com = GdbCom::getInstance();
    Tree resultData;
    int rc = 0;
...
int GdbCore::initRemote(Settings *cfg, QString gdbPath, QString programPath, QString tcpHost, int tcpPort)
{
    GdbCom& com = GdbCom::getInstance();
    Tree resultData;
 ...
 */
void GdbCore::writeTargetStdin(QString text)
{
...
void GdbCore::onGdbOutput(int socketFd)
{
    Q_UNUSED(socketFd);
    char buff[128];
    buff[0] = '\0';
 ...
 * @return 0 on success.
 */
int GdbCore::gdbGetMemory(quint64 addr, size_t count, QByteArray *data)
{
    GdbCom& com = GdbCom::getInstance();
    Tree resultData;
 ...
 */
int GdbCore::gdbSetBreakpointAtFunc(QString func)
{
    GdbCom& com = GdbCom::getInstance();
    Tree resultData;
    int rc = 0;
 ...
 * @return 0 on success.
 */
int GdbCore::jump(QString filename, int lineNo)
{
    int res;
    GdbCom& com = GdbCom::getInstance();
    Tree resultData;
 ...
 * @return 0 on success.
 */
int GdbCore::evaluateExpression(QString expr, QString *data)
{
    GdbCom& com = GdbCom::getInstance();
    Tree resultData;
    GdbResult res;
 ...
 */
VarWatch* GdbCore::getVarWatchInfo(QString watchId)
{
    assert(watchId != "");
    assert(watchId[0] == 'w');
    for(int i = 0;i < m_watchList.size();i++)
 ...
 */
QList <VarWatch*> GdbCore::getWatchChildren(VarWatch &parentWatch)
{
    QList <VarWatch*> list;
    for(int i = 0;i < m_watchList.size();i++)
    {
 ...
 * @return 0 on success.
 */
int GdbCore::gdbAddVarWatch(QString varName, VarWatch** watchPtr)
{
    GdbCom& com = GdbCom::getInstance();
    Tree resultData;
    QString watchId;
    ...
    *watchPtr = NULL;
    ...
    setValueFromGdbString(w, varValue2);
 ...
 * @return 0 on success.
 */
int GdbCore::gdbExpandVarWatchChildren(QString watchId)
{
    int res;
    Tree resultData;
    GdbCom& com = GdbCom::getInstance();
    ...
    for(int i = 0;i < root->getChildCount();i++)
    {
        ...
        if(watch == NULL)
        {
            ...
            setValueFromGdbString(watch, childValue);
 ...
 */
QString GdbCore::gdbGetVarWatchName(QString watchId)
{
 ...
 */
void GdbCore::gdbRemoveVarWatch(QString watchId)
{
    GdbCom& com = GdbCom::getInstance();
    Tree resultData;
...
void GdbCore::onNotifyAsyncOut(Tree &tree, AsyncClass ac)
{
...
ICore::StopReason GdbCore::parseReasonString(QString reasonString)
{
...
void GdbCore::onExecAsyncOut(Tree &tree, AsyncClass ac)
{
    GdbCom& com = GdbCom::getInstance();
...
void GdbCore::gdbRemoveAllBreakpoints()
{
    ensureStopped();
 ...
 */
void GdbCore::gdbRemoveBreakpoint(BreakPoint* bkpt)
{
    GdbCom& com = GdbCom::getInstance();
    Tree resultData;
 ...
 */
BreakPoint* GdbCore::findBreakPoint(QString fullPath, int lineNo)
{
    for(int i = 0;i < m_breakpoints.size();i++)
    {
        BreakPoint *bkpt = m_breakpoints[i];
 ...
 */
BreakPoint* GdbCore::findBreakPointByNumber(int number)
{
    for(int i = 0;i < m_breakpoints.size();i++)
    {
        BreakPoint *bkpt = m_breakpoints[i];
...
void GdbCore::dispatchBreakpointDeleted(int id)
{
...
void GdbCore::dispatchBreakpointTree(Tree &tree)
{
...
void GdbCore::onResult(Tree &tree)
{
        ...
        if(rootName == "changelist")
         {
            ...
            for(int j = 0;j < rootNode->getChildCount();j++)
            {
                ...
                setValueFromGdbString(watch, child->getChildDataString("value"));
...
void GdbCore::onStatusAsyncOut(Tree &tree, AsyncClass ac)
{
    infoMsg("StatusAsyncOut> %s", GdbCom::asyncClassToString(ac));
    tree.dump();
}

void GdbCore::onConsoleStreamOutput(QString str)
{
    str.replace("\r","");
...
void GdbCore::onTargetStreamOutput(QString str)
{
    str.replace("\r","");
    QStringList list = str.split('\n');
    for(int i = 0;i < list.size();i++)
...
void GdbCore::onLogStreamOutput(QString str)
{
    str.replace("\r","");
    if(str.endsWith('\n'))
 ...
 */
int GdbCore::gdbSetBreakpoint(QString filename, int lineNo)
{
    GdbCom& com = GdbCom::getInstance();
    Tree resultData;
    int rc = 0;
 ...
 */
QList<ThreadInfo> GdbCore::getThreadList()
{
 ...
 */
void GdbCore::selectThread(int threadId)
{
 ...
 */
void GdbCore::selectFrame(int selectedFrameIdx)
{
 ...
 */
int GdbCore::changeWatchVariable(QString watchId, QString newValue)
{
    QString dataStr;
    GdbCom& com = GdbCom::getInstance();
    Tree resultData;
 ...
 */
quint64 GdbCore::getAddress(VarWatch &w)
{
    int rc = 0;
    quint64 addr = 0;
    GdbResult gdbRes = GDB_ERROR;
